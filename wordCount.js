...
app.get("/test", function(req, res) {
  ...
  Archive.aggregate(
    [{ $match: { siteID: 10 } }, { $project: { headline: 1 } }],
    function(error, documents) {
      if (error) {
        ...
        let headlineWordsArr = headlinesArr.flat(Infinity);

        // Alternate method for flattening array where split method is not used in the reduce method but applied to the flatMap method
        // console.log (headlinesArr.flatMap(document => document.split(" ")));
        ...
        let wordCountObj = headlineWordsArrNoStopWords.reduce(
          (allWords, word) => {
            if (word in allWords) {
              allWords[word]++;
            } else {
        ...
        console.log(sortedWordCountArr);

        res.send(sortedWordCountArr);
...
// // // Get all headlines from archive DB
// Archive.aggregate(
//   [{ $match: { siteID: 3 } }, { $project: { headline: 1 } }],
//   function(error, documents) {
//     if (error) {
//       console.log(error);
//     } else {
...
//       // Clean up array by removing stop words - using stopword npm package --> https://www.npmjs.com/package/stopword
//       const headlineWordsArrNoStopWords = sw.removeStopwords(headlineWordsArr);
...
// let test = arr1.flatMap(x => x.split(" "));
